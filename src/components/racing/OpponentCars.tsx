...
const OpponentCars: React.FC<OpponentCarsProps> = ({ opponentCars }) => {
  console.log('🚗 Renderizando carros adversários:', opponentCars.length);
  
  return (
    <>
      {opponentCars.map((car) => {
        console.log(`🚙 Carro ID ${car.id}: Lane ${car.lane + 1}, Pos ${car.position.toFixed(1)}%`);
        ...
        return (
          ...
          >
            ...
            <div className="relative w-full h-full">
              <div 
                className="absolute inset-0 rounded-lg shadow-2xl border-2 border-white border-opacity-30"
                style={{ 
              ...
              >
                {/* Para-brisa */}
                <div className="absolute left-1/4 right-1/4 top-2 h-5 bg-gray-100 bg-opacity-90 rounded-t" />
                ...
                <div className="absolute -top-6 left-1/2 transform -translate-x-1/2 bg-red-600 text-white text-xs px-1 rounded">
